...
class Bikestore 
    ...
    def parse_bikestore_page
        ...
        # Descriptions
        @bike_desc = @prodlist.find_elements(DESC_LOCATOR)
        @bike_desc.map!(&:text) # get text
    ...
    def bike_images_present?
        parse_bikestore_page
        parse_json_file
        @bike_images.length >= 0 # There are some.
    end
    ...
    def bike_images_match?
        ...
        @bike_images == @image_json
    end
    
    def bike_classes_present?
        parse_bikestore_page
        parse_json_file
        @bike_classes.length >= 0
    end
    ...
    def bike_classes_match?
        ...
        # Appears that bikes classes on page are being given a Capital Letter, lowercase for comparison:
        @bike_classes.map! { |category|
            category.map!(&:downcase)
    }
        @bike_classes == @class_json
    end
